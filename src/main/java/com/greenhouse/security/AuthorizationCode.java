...
public class AuthorizationCode {
    ...
    private static final String codePrefix = "urn:smartgreenhouse:code:";
    ...
    public String getCode(String codeChallenge) throws Exception {
        ...
        return code + ":" + Base64.getEncoder().withoutPadding()
                .encodeToString(ChaCha20Poly1305.encrypt(codeChallenge.getBytes(), key));
    }
    ...
    public static AuthorizationCode decode(String authorizationCode, String codeVerifier) throws Exception {
        ...
        if (!expected.equals(new String(ChaCha20Poly1305.decrypt(Base64.getDecoder().decode(cipherCodeChallenge), key),
                StandardCharsets.UTF_8).replace('_', '/').replace('-', '+'))) {
            System.out.println(expected + " ?= "
                    + new String(ChaCha20Poly1305.decrypt(Base64.getDecoder().decode(cipherCodeChallenge), key),
